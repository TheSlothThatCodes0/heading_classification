...
# now non‑capturing for glue words:
...
def parse_pdf_and_merge_lines_v2(pdf_path: Path):
    ...
    for page_idx, page in enumerate(doc):
        ...
        if not lines:
            continue
        # sort + group very similarly to original...
...
def infer_pdf(pdf_path: Path):
    ...
    # Rule 1: font-size threshold
    feats = feats[feats["relative_font_size"] >= 1.1]

    # If nothing remains, return empty outline
    if feats.empty:
        return {"title": "", "outline": []}

    # exactly these 11 features
    FEATURE_COLS = [
    ...
    # Rule 3: remove repeated noise
    tp   = hdrs[["text","page_num"]].drop_duplicates()
    freq = tp.groupby("text").size()
    noise_texts = set(freq[freq>3].index)
    hdrs = hdrs[~hdrs["text"].isin(noise_texts)]

    # Rule 4: sentence/body filters
    hdrs = hdrs[~hdrs["text"].str.match(date_pattern)]
    hdrs = hdrs[~hdrs["text"].str.match(page_footer_pattern)]
    ...
    if not p1.empty:
        mx = p1["font_size"].max()
        title = " ".join(p1[p1["font_size"] == mx]["text"])
    else:
...
def main():
    ...
    try:
        ...
        print(f"❌ Could not load from {MODEL_PATH} or {ENCODER_PATH}")
        return
